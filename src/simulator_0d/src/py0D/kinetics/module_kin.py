...
import simulator_0d.src.py0D.kinetics.enthalpy_numba as enth_nb
import simulator_0d.src.py0D.kinetics.functions_numba as func_nb
import simulator_0d.src.py0D.kinetics.int_foo as ifo
import simulator_0d.src.py0D.kinetics.second as sec
from simulator_0d.src.py0D.kinetics.int_foo import add_method, add_attr
...
def kin_dQ_dt(alpha, Q, dQ_dt_0D, dt, int_BDF = None, T = None, bool_recompute_T = True):
    # chron1 = time.time()
    if bool_recompute_T:
        T = enth_nb.find_g_temperature(np.array([T]), np.array([Q]))[0]
    y0 = (Q)
...
def init_BDF(T, alpha, Q, dQ_dt_0D, dt, bool_get_hist = False):
    y0 = (Q)
    y0 = np.insert(y0, 0, alpha)
    y0 = np.insert(y0, 0, T)
    ...
    args = (dQ_dt_0D,)
    rtol    = 1e-3
    atol = np.full(y0.shape, 1e-3)
    atol[(gd.i_Y+2):] = 1e-10

    int_BDF = BDF(fun_to_integrate, 0, y0, dt, atol=np.array(atol), rtol=rtol)
    add_attr(int_BDF, dict(bool_get_hist = bool_get_hist))
    add_method(int_BDF)

    return int_BDF


def compute_kin(gas, dt, volume, int_BDF = None, bool_get_hist = False):
    ...
    if int_BDF is None:
        int_BDF = init_BDF(gas.temperature, gas.alpha, Q, dQ_dt_0D, dt, bool_get_hist = bool_get_hist)
    ...
    (t_BDF,
        ...
        int_BDF) = kin_dQ_dt(gas.alpha,
                                ...
                                int_BDF = int_BDF,
                                T = gas.temperature,
                                bool_recompute_T = False
                                )

    # (t_BDFn,
    #     Q_BDFn,
    #     int_BDFn) = kin_dQ_dt_normal_int_foo(gas.alpha,
    #                                         Q,
    #                                         dQ_dt_0D,
    #                                         dt,
    #                                         )
...
def kin_dQ_dt_normal_int_foo(alpha, Q, dQ_dt_0D, dt, T = None):
    if T is None:
    ...
    rtol    = 1e-3
    atol    = 1e-1
    atol = np.full(y0.shape, 10)
    atol[(gd.i_Y+2):] = 1e-10

    def fun(t,y):
        return fun_to_integrate(t,y,*args)

    int_BDF = BDF(fun, 0, y0, dt, atol=np.array(atol), rtol=rtol)
    add_method(int_BDF)
