...
import py0D.kinetics.perso_scipy_jac_modified as psjm
...
# @jit
def fun_wrapped_nb(fun, t, y):
    return np.asarray(fun(t, y))


# @jit
def fun_vectorized(fun, t, y):
    f = np.empty_like(y)
...
def init_BDF(int_BDF, y0, dt, t0=0, fun_wo_arg=None, args = None):
############################### init ode simplified #############################################""
    ...
    def fun(t, y):
        int_BDF.nfev += 1
        return int_BDF.fun_single(t, y)

    int_BDF.fun = fun
    int_BDF.fun_single = fun_single
...
############################### init BDF simplified #############################################""
    f = int_BDF.fun(int_BDF.t, int_BDF.y)
    int_BDF.h_abs = select_initial_step(int_BDF.fun, int_BDF.t, int_BDF.y, f,
                                        int_BDF.direction, 1,
                                        int_BDF.rtol, int_BDF.atol)
    ...
    if "fun_vectorized_nb" not in int_BDF.__dict__.keys():
        ...
        int_BDF.fun_single_unwrapped = fun
...
def _validate_jac_new(int_BDF, jac, sparsity):
    ...
    if jac is None:
        def jac_wrapped(t, y):
            int_BDF.njev += 1
            f = int_BDF.fun_single(t, y)
            J, int_BDF.jac_factor = psjm.num_jac_nb(int_BDF.fun_vectorized_nb,int_BDF.fun_single_unwrapped,
                                            t, y, f,
