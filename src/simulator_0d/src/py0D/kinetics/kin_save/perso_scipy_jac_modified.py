...
# @njit
def num_jac_nb(fun, fun_unwrapped, t, y, f, threshold, factor, sparsity=None):
   ...
   #     factor = factor.copy()
    ...
    # chron1 = time.time()
    A = dense_num_jac_new_nb(fun,fun_unwrapped, t, y, f, h, factor, y_scale, n)
...
# @njit
def dense_num_jac_new_nb(fun, fun_unwrapped, t, y, f, h, factor, y_scale, n):
    check_go = True
    cpt = 0
    while check_go:
        h_vecs = np.diag(h)
        f_new = fun(fun_unwrapped, t, add_dim(y) + h_vecs)
    ...
    if np.any(diff_too_small):
        ...
        for j in range(len(h_new)):
            for i in range(len(ind)):
                if i == ind[j]:
                    h_vecs[i,i] = h_new[j]
        f_new = fun(fun_unwrapped, t, add_dim(y) + h_vecs[:, ind])
...
def _step_impl(int_BDF):
    ...
    if int_BDF.h_abs > max_step:
        h_abs = max_step
        change_D(D, int_BDF.order, max_step / int_BDF.h_abs)
        int_BDF.n_equal_steps = 0
    elif int_BDF.h_abs < min_step:
        h_abs = min_step
        change_D(D, int_BDF.order, min_step / int_BDF.h_abs)
        int_BDF.n_equal_steps = 0
    else:
        h_abs = int_BDF.h_abs
        ...
        if not (y_predict[(gd.i_Y + 2) :]<0).any():
            ...
            while not converged:
                ...
                converged, n_iter, y_new, d = solve_bdf_system(
                    int_BDF.fun, t_new, y_predict, c, psi, LU, int_BDF.solve_lu,
                    scale, int_BDF.newton_tol)
