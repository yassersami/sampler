...
def append_hypercube_boundary_points(data, features, n_per_dim) -> pd.DataFrame:
    """
    Append points on the boundary of a p-dimensional hypercube [0, 1]^p
    with k values per dimension.

    :param features: label for each dimension
    :param n_per_dim: number of values per dimension
    :return: Data with new points on the hypercube boundary
    """
    ...
    n_dim_X = len(features)
    all_points = list(itertools.product(values, repeat=n_dim_X))
    ...
    df_boundary = pd.DataFrame(boundary_points, columns=features)
